'''
Evaluation script supports single GPU
and saves predictions
in panoptic format (trainId)
'''
...
def make_dataloader(args, config):
...
def save_prediction_image(_, panoptic_pred, img_info, out_dir, colors, num_stuff):
    ...
    ## sem add inst
    sem_new = np.zeros_like(sem, dtype=np.int32)
    sem[msk!=0] = 0
    sem_new += cat * 1000 + msk + sem
    Image.fromarray(sem_new).save(out_path.replace('_leftImg8bit.png', '_gtFine_instanceIds.png'))
    '''
